...
class convertTemplate(MethodResource):
    ...
    def getGeneralInfo(self, sheet):
        ...
        addData = lambda name, val, d: d.update( {name: val} ) if val != "#NULL#" else False
        keys = ["name", "doc", "doi", "pubmedID", "url"]

        for i, name in enumerate(keys): addData( name, sheet.iloc[i, 1], data )
    ...
    def getVessels(self, sheet, enzmldoc):
        ...
        enzmldoc.setVessel( Vessel( id_ = "v0", **data[ "v0" ] ) )
    ...
    def getReactants(self, sheet, enzmldoc):
        ...
        # Reactant(name, compartment, init_conc=0.0, substanceunits='NAN', constant=False, smiles=None, inchi=None)
        # rename columns to match kwargs
        sheet.columns = ["id_", "name", "compartment", "constant", "smiles", "inchi"]
        sheet = sheet.set_index("id_")
        
        data = sheet.to_dict("index")
        
        # Create PyEnzyme object
        boolCheck = lambda val: bool(val) if val in "Not constant Constant" else val
    ...
    def getProteins(self, sheet, enzmldoc):
        ...
        # rename columns to match kwargs
        sheet.columns = ["id_", "name", "sequence", "compartment", "constant", "organism", "ecnumber", "uniprotid"]
        sheet = sheet.set_index("id_")
        
        data = sheet.to_dict("index")
        
        # Create PyEnzyme object
        boolCheck = lambda val: True if val == "Not constant" else False if val == "Constant" else val
    ...
    def getReactions(self, sheet, enzmldoc):
        ...
        # EnzymeReaction(self, temperature, tempunit, ph, name, reversible, educts=None, products=None, modifiers=None)
        # rename columns to match kwargs
        sheet.columns = ["id_", "name", "temperature", "tempunit", "ph", "reversible", "educts", "products", "proteins", "modifiers"]
        sheet = sheet.set_index("id_")
        
        data = sheet.to_dict("index")
        
        # Create PyEnzyme object
        boolCheck = lambda val: True if val == "reversible" else False if val == "irreversible" else val
        
        # rearrange unison of protein/reactantDict to guarantee consistency
        inv_prDict = { name: id_ for id_, name in {**self.proteinDict, **self.reactantDict}.items() }
                ...
                for elem in elements[key]:
                    ...
                    if elem != "#NULL#":
                        ...
                        if 's' in elem_id:
                            ...
                            raise KeyError(f"The identifier {elem_id} could not be parsed. Make sure the ID is either s for reactants or p for proteins.")
            ...
            for exp in exp_ids:
                ...
                for index, row in df_exp.iterrows():
                    ...
                    if "Time" in datType:
                        ...
                        time_raw = [ val for val in list( row.iloc[9::] ) if type(val) == float ]
                        ...
                        if len(data_raw) > 0:
                            ...
                            enzmldoc.getReaction(reac, by_id=False).addInitConc(reactant, init_val, init_unit, enzmldoc)
                ...
                for i, reactant in enumerate( exp_dict["reactants"] ):
                    ...
                    enzmldoc.getReaction(reac, by_id=False).addReplicate( repl, enzmldoc )