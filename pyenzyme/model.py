...
class EnzymeMLDocument(BaseModel):
    ...
    conditions: Optional[ReactionConditions] = Field(default=None)
    ...
    def filter_equations(self, **kwargs) -> list[ODE]:
        ...
        Returns:
            list[ODE]: The filtered list of ODE objects
        """

        return FilterWrapper[ODE](self.equations, **kwargs).filter()
...
class Creator(BaseModel):
    ...
    # JSON-LD fields
    ld_id: str = Field(
        serialization_alias="@id", default_factory=lambda: "md:Creator/" + str(uuid4())
    )
    ld_type: list[str] = Field(
        serialization_alias="@type",
...
class Vessel(BaseModel):
    ...
    # JSON-LD fields
    ld_id: str = Field(
        serialization_alias="@id", default_factory=lambda: "md:Vessel/" + str(uuid4())
    )
    ld_type: list[str] = Field(
        serialization_alias="@type",
...
class Complex(BaseModel):
    ...
    # JSON-LD fields
    ld_id: str = Field(
        serialization_alias="@id", default_factory=lambda: "md:Complex/" + str(uuid4())
    )
    ld_type: list[str] = Field(
        serialization_alias="@type",
        default_factory=lambda: [
...
class Reaction(BaseModel):
    ...
    name: str
    reversible: bool = False
    rate_law: Optional[Equation] = Field(default=None)
    species: list[ReactionElement] = Field(default_factory=list)
    modifiers: list[str] = Field(default_factory=list)

    # JSON-LD fields
    ld_id: str = Field(
        serialization_alias="@id", default_factory=lambda: "md:Reaction/" + str(uuid4())
    )
    ld_type: list[str] = Field(
        serialization_alias="@type",
        default_factory=lambda: [
...
class ReactionElement(BaseModel):
    ...
    species_id: str
    stoichiometry: Optional[float] = Field(default=None)
...
class ODE(BaseModel):
    ...
    # JSON-LD fields
    ld_id: str = Field(
        serialization_alias="@id", default_factory=lambda: "md:ODE/" + str(uuid4())
    )
    ld_type: list[str] = Field(
        serialization_alias="@type",
        default_factory=lambda: [
...
class Equation(BaseModel):
    ...
    # JSON-LD fields
    ld_id: str = Field(
        serialization_alias="@id", default_factory=lambda: "md:Equation/" + str(uuid4())
    )
    ld_type: list[str] = Field(
        serialization_alias="@type",
        default_factory=lambda: [
