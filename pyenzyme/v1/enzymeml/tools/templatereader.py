...
def read_template(path: str, enzmldoc):
    ...
    for experiment_id in experiment_ids:
        ...
        # Get time
        time_values, time_unit = get_time(exp_data, split_index)  # type: ignore
...
def extract_values(sheet: pd.DataFrame, mapping: Dict[str, str]) -> list:
    ...
    return [
        {
            mapping.get(key): item  # type: ignore
            for key, item in record.items()
            if item and mapping.get(key) and "nan" != repr(item)  # type: ignore
        }
