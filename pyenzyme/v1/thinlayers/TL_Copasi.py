...
class ThinLayerCopasi(BaseThinLayer):
    ...
    def _import_experiments(self):
        ...
        for measurement_id, measurement_dict in self.data.items():
            ...
            exp_filename = os.path.abspath(
                os.path.join(self.working_dir, measurement_id + ".tsv")
            )

            data.to_csv(exp_filename, sep="\t", header=True, index=False)

            exp = COPASI.CExperiment(self.dm)
            exp.setObjectName("{0}".format(measurement_id))
            exp.setFirstRow(1)
    ...
    def get_fit_parameters(self):
        ...
        for i in range(self.problem.getOptItemSize()):
            ...
            name = (
                ...
                else obj.getObjectParent().getObjectName()
            )
    ...
    def optimize(self, update_model=False):
        ...
        if not self.task.initializeRaw(COPASI.CCopasiTask.OUTPUT_UI):  # type: ignore
    ...
    def _set_default_items_from_init_file(self):
        ...
        for global_param in self.global_parameters.values():
            ...
            value = (
                global_param.value if global_param.value else global_param.initial_value
            )
...
if __name__ == "__main__":
    ...
    thin_layer = ThinLayerCopasi(path=filename, outdir="./out")
