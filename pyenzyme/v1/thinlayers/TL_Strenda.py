...
from pyenzyme.v1.enzymeml.models.michaelismenten import MichaelisMentenKCat
...
class ThinLayerStrendaML(object):
    ...
    def toEnzymeML(cls, path: str, out_dir: str):
        ...
        vessel = Vessel(name="Vessel")  # type: ignore
        cls.vessel_id = cls.enzmldoc.addVessel(vessel)  # type: ignore
    ...
    def _setup_measurements(self, init_conc: List[Tuple[str, float, str]]):
        ...
        for index, init_conc_set in enumerate(itertools.product(*init_conc)):
            ...
            measurement = Measurement(name=f"measurement_{index + 1}")  # type: ignore
            ...
            measurement.addData(
                protein_id="p0",
                init_conc=protein_conc,  # type: ignore
                unit=protein_unit.replace("micro", "u"),  # type: ignore
            )

            for species_id, value, unit in init_conc_set:  # type: ignore
