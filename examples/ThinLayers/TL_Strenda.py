...
-----
Last Modified: Wednesday June 23rd 2021 7:18:29 pm
...
class ThinLayerStrendaML(object):
        ...
        for i, dataset in enumerate(datasets):
            ...
            reaction.setName(
                    ...
                    (
                        ...
                        self.enzmldoc.getUnitDict()[
                            self.enzmldoc.getProtein(f"p{i}").getSubstanceUnits()
                            ].getName()
                    )
            ...
            for small_comp in small_comps:
                ...
                if "Range" in small_comp.find("value").attrib["type"]:
                    ...
                    conc = np.linspace(
                        float(small_comp.find("value").attrib["startValue"]),
                        float(small_comp.find("value").attrib["endValue"]),
                        4
                    ).tolist()

                    unit = small_comp.find("value").attrib["unit"]
                    initConcs = [(c, unit) for c in conc]

                else:

                    conc = [float(small_comp.find("value").attrib["value"])]
                    unit = small_comp.find("value").attrib["unit"]
                    initConcs = [(conc[0], unit)]

                reactant = Reactant(name, "v0", conc[0], unit, False)

                if inchi != "null":
                    reactant.setInchi(inchi)
                if smiles != "null":
                    reactant.setSmiles(smiles)
                ...
                if small_comp.attrib["role"] != "Substrate":
                    ...
                    reaction.addModifier(
                        id_, 1.0, True, self.enzmldoc, [], initConcs
                    )

                else:
                    reaction.addEduct(
                        id_, stoich, False, self.enzmldoc, [], initConcs
                    )
            ...
            km = KineticModel(
                f"kcat_{sub_id}*p{i}*{sub_id} / ( km_{sub_id} + {sub_id} )",
                parameters,
                self.enzmldoc
            )
    ...
    def __getProtein(self, root):
        ...
        for dataset in datasets:
            ...
            init_conc = float(
                dataset.find("assayConditions")[0].attrib["value"]
            )
            ...
            if prot_node.attrib["uniprotKbAC"] != 'N.A.':
                protein.setUniprotID(prot_node.attrib["uniprotKbAC"])
